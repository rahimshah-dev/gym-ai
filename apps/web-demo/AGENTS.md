...
- The Gemini API key stays server-side in `server.js` — never move it into `index.html` client-side code. This was already fixed once (see git log, originally for OpenAI, same rule applies after the Gemini swap); don't regress it.
